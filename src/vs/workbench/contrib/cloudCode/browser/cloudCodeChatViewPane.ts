...
import { CloudCodeAgentExecutionFactory } from './cloudCodeAgentExecution.js';
...
export class CloudCodeChatViewPane extends ViewPane {
	...
	constructor(
		...
		this.agent = new CloudCodeAgent(cloudCodeService, agentWorkspace, this.editWorkspace, editingWorkspace, instantiationService.createInstance(CloudCodeAgentExecutionFactory));
	...
	protected override renderBody(container: HTMLElement): void {
		...
		this.widget = this._register(new CloudCodeChatWidget(container, async (models, selected) => {
			...
			const items: { id: CloudCodeChatMode; label: string; description: string }[] = [
				{ id: 'agent', label: localize('cloudcode.agentMode', "Agent"), description: localize('cloudcode.agentModeDetail', "Edit across files, run approved checks, and fix failures") },
