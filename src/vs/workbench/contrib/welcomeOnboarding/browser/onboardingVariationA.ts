...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		if (this.overlay || product.disableBuiltinCopilot || !defaultChat) {
			return;
		}
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to CloudCode"));
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to CloudCode");
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all CloudCode commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // CloudCode default, nothing to install
		}
